...
def save_record(player, start_time, outcome):
    mins = round((time.time() - start_time) / 60, 2)
    try: #include try to take account for file errors
      with open(RECORDS_FILE, "a") as f:
          f.write(f"{player} - {mins} min - {outcome}\n")
      print(f"\nTime: {mins} min | Saved to {RECORDS_FILE}")
    except OSError as x:
        print(f"couldnt save record due to {x}")
...
def use(item_name, player, start_time):
    item = find_item(item_name, inventory)
    if not item:
    ...
    if name == "axe":
        inventory.remove(item)
        delay(
            "You use the axe to chop down a small tree. Now you have some wood, but the axe broke...and its hard to carry loose logs...")
        inventory.append({"name": "wood", "type": "material",
                          "description": "some chopped logs."})  # axe breaks but wood is added to inventory
        ...
        if wood is not None:  # i used chatai for these two lines as i tried to work with boolean, which wasnt possible
            ...
            inventory.append({"name": "bundle", "type": "material",
                              "description": "Bundled wood"})  # "merges" wood and rope to bundle
        else:
        ...
        ==================
         """)
        save_record(player, start_time, "LOST")
        return True
        # ends the game
        exit()
        ...
        if torch is not None:
            ...
            You win!!!
            =========================""")
            save_record(player, start_time, "WIN")
            return True
            exit()
...
def game_loop():
    ...
    if DEBUG == True:
        save_record(player, 0, "SKIP")
        exit()

    time.sleep(1)
    delay("""You are a simple villager of ralzica. It is getting close to winter and you want to gather some wood. 
    You go into the forest near your house, where you have already layed out some useful items this morning and left them there \n""")
    time.sleep(1)
    print("\n", player, ". Be quick and get home safe.", """
        ========================================================================
    ...
    while True:
        ...
        match command.split():
            ...
            case ["drop", item_name]:
                drop(item_name)
            case ["use", item_name]:
                use(item_name, player, start_time)  #chatai helped with this line as i didnt know i needed to pass i.e player to the functions here
            case ["examine", item_name]:
                examine(item_name)
            case ["quit"]:
                save_record(player, start_time, "QUIT")
